import { EnemyPartyConfig, generateModifierTypeOption, initBattleWithEnemyConfig, leaveEncounterWithoutBattle, selectOptionThenPokemon, selectPokemonForOption, setEncounterRewards, } from "#app/data/mystery-encounters/utils/encounter-phase-utils";
import { trainerConfigs, TrainerPartyCompoundTemplate, TrainerPartyTemplate, TrainerSlot, } from "#app/data/trainer-config";
...
import { AttackTypeBoosterHeldItemTypeRequirement, CombinationPokemonRequirement, HeldItemRequirement, TypeRequirement } from "#app/data/mystery-encounters/mystery-encounter-requirements";
import { Type } from "#app/data/type";
import { AttackTypeBoosterModifierType, ModifierTypeOption, modifierTypes } from "#app/modifier/modifier-type";
import { AttackTypeBoosterModifier, BypassSpeedChanceModifier, ContactHeldItemTransferChanceModifier, PokemonHeldItemModifier } from "#app/modifier/modifier";
  ...
  MysteryEncounterBuilder.withEncounterType(MysteryEncounterType.BUG_TYPE_SUPERFAN)
    ...
    .withOption(MysteryEncounterOptionBuilder
      ...
      .withOptionPhase(async (scene: BattleScene) => {
        ...
        // TODO: add golden bug net
        setEncounterRewards(scene, { guaranteedModifierTypeFuncs: [modifierTypes.REVIVER_SEED], fillRemaining: false });
...
function getTrainerConfigForWave(waveIndex: number) {
  ...
  config.name = i18next.t("trainerClasses:bug_type_superfan");
...
function doBugTypeMoveTutor(scene: BattleScene): Promise<void> {
  return new Promise<void>(async resolve => {
    ...
    let result = await selectOptionThenPokemon(scene, optionSelectItems, `${namespace}.option.3.select_prompt`, undefined, onHoverOverCancel);
